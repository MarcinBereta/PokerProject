...
class GameGui:
    ...
    def on_closing(self):
        if self.game_socket_handler.winner:
    ...
    def update(self):
        if self.game_socket_handler.game_data is not None:
        ...
        if self.game_socket_handler.winner:
            ...
            for player in players_at_table.keys():
                players_at_table[player] = (players_at_table[
        ...
        if self.game_socket_handler.newUpdate:
            ...
            actual_player = (players_at_table[
                                 self.actual_id] - self.shift + self.starting_players) % self.starting_players
        ...
        if self.move is not None and self.actual_id == self.user_id:

...
class GameGui:
    ...
    def update(self):
        if self.shift is None and self.game_socket_handler.game_status is not None:
            ...
            for player in self.seats.keys():
                self.shifted_players[player] = (self.seats[player] - self.shift + self.starting_players) % self.starting_players
            
        if self.starting_players is None:
            self.starting_players = len(self.nick)
        
        if self.game_socket_handler.game_status is not None and self.game_socket_handler.game_status['winner'] is not None:
            self.handle_winner() 
        ...
        if self.game_socket_handler.game_status is not None and self.game_socket_handler.game_status['winner'] is None:
            ...
            # print(f"[NEW DATA]{self.game_socket_handler.game_status}\n")
            ...
            # for i in self.board_labels:
            #     self.delete_card(i,True)
            ...
            # if self.user_id in self.seats.keys():
            #     self.display_card('table00card01', self.game_socket_handler.cards[0])
            #     self.display_card('table00card02', self.game_socket_handler.cards[1])
    ...
    def open_popup(self):
        ...
        confirm.pack()
        tk.Label(top, text="Winner is " + str(self.game_socket_handler.game_status['winner']), font=('Mistral 18 bold')).place(x=75, y=75)
